...
const AdminNotificaciones = ({ data, onRefresh }: { data: AdminData; onRefresh: () => void }) => {
  ...
  const fetchHistorial = useCallback(async () => {
    ...
    const { data: rows, error } = await supabase
      ...
      .select("id, created_at, client_id, tipo, asunto, email_to, estado")
      .order("created_at", { ascending: false })
      .limit(50);

    if (!error && rows) setHistorial(rows as NotifRow[]);
  ...
  const sendNotification = async () => {
    ...
    const client = clients.find((c) => c.user_id === selectedClient);
    const email_to = client?.email;

    if (!email_to) {
      toast.error("El cliente seleccionado no tiene email registrado");
      return;
    }
    ...
    try {
      ...
      if (res.error || res.data?.error) {
        ...
        toast.success(`✉️ Email enviado a ${client?.nombre}`);
        setSelectedClient("");
        setTipo("");
        setAsunto("");
        setMensaje("");
    ...
    <div>
      ...
      <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-6 mb-8">
        ...
        <div className="mb-4">
          ...
          <textarea
            className={`${darkInput} h-32 resize-y`}
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <button
            ...
            disabled={sending}
            className="bg-primary text-white rounded-lg px-4 py-2 text-sm font-body cursor-pointer border-none disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {sending ? (
              <>
          ...
          {sending && (
            <span className="text-xs text-white/40">Esto puede tardar unos segundos…</span>
          )}
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-white/[0.06]">
                {["Fecha", "Cliente", "Tipo", "Asunto", "Estado"].map((h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-xs font-semibold text-white/40 uppercase tracking-wide"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {loadingHistorial ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-white/28 text-sm">
                    Sin notificaciones enviadas
                  </td>
                </tr>
              ) : (
                historial.map((n) => {
                  const client = clients.find((c) => c.user_id === n.client_id);
                  return (
                    <tr key={n.id} className="border-t border-white/[0.04]">
                      ...
                      <td className="px-4 py-3 text-sm text-white/70">
                        {client?.nombre ?? n.email_to}
                      </td>
                      <td className="px-4 py-3">
                        <span className="bg-primary/15 text-blue-light px-2.5 py-0.5 rounded text-xs font-semibold">
                          {n.tipo}
                        </span>
                      </td>
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-white">
          ...
          {unreadCount > 0 && (
            <span className="bg-danger text-white text-xs px-2 py-0.5 rounded-full ml-2">
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            className="bg-blue-light/15 text-blue-light border border-blue-light/30 rounded-lg px-3 py-1 text-xs font-body cursor-pointer"
          >
            ✓ Marcar todas leídas
          </button>
        )}
              ...
              <div
                ...
                className={`rounded-xl p-5 relative border ${
                  s.leida
                    ...
                    : "bg-blue-light/[0.08] border-blue-light/30"
                }`}
              >
                {!s.leida && (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap mb-3">
                  <div>
                    ...
                    <div className="text-xs text-white/45 mt-0.5">
                      {client?.empresa} · {client?.telefono || "—"}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {s.score != null && (
                      ...
                      >
                        {s.score} pts (
                        {s.nivel === "high" ? "Alto" : s.nivel === "medium" ? "Medio" : "Bajo"})
                      </span>
                    )}
                    <button
                      onClick={() => markRead(s.id)}
                      className="bg-white/[0.07] text-white/50 border-none rounded-md px-2 py-0.5 text-xs cursor-pointer"
                    >
                ...
                <div className="flex gap-4 text-xs text-white/35">
                  ...
                  {s.disponibilidad && (
                    <span>
                      📅 Disponibilidad:{" "}
                      <strong className="text-white/55">{s.disponibilidad}</strong>
                    </span>
                  )}
                </div>
              </div>
